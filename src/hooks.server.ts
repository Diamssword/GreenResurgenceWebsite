...
if (process.env.ENABLE_BOT?.toLowerCase() == "true") import("$lib/bot/bot").catch(console.error);
else console.log("ENABLE_BOT not set to 'true', skipping...");
...
export const handle: Handle = async ({ event, resolve }) => {
  ...
  if (!event.locals.user_ip) event.locals.user_ip = "devIP";
...
function clearCache() {
  ...
  } else fs.mkdirSync("./uploaded/cache", { recursive: true });
