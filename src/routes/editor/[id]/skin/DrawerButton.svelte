    ...
    <Dropdown  class="ml-2 p-0">
        <div class="p-1">
            {#each texture.subs as sub }
                <Button class="ml-2 p-0 cursor-pointer {picked==sub.id?"bg-secondary-text hover:bg-secondary-text2":""}" onclick={()=>{onclick(sub); picked=sub.id}}><img class="size-14" src="/skins/display/{skinEditor.getTextureIconPath(layer,sub.id,texture.id,category)}" alt={sub.name||sub.id}/></Button>
            {/each}
        </div>
    </Dropdown>
{/if}
