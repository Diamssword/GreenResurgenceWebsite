...
<div class="my-3">
    ...
    {#if skin.id=="clear"}
        <Button class="ml-2 p-2 -translate-y-2" onclick={()=>skinEditor.clearPart(layer)}><img class="size-10" src="/skins/display/clear.svg" alt="Aucun"/></Button>    
        <Tooltip type="light">Aucun</Tooltip>
    {:else}
        <Button class="ml-2 p-0" onclick={()=>skinEditor.pickTexture(layer,skin.id)}><img class="size-14" src="/skins/display/{skinEditor.getTexturePath(layer,skin.id)}" alt={skin.name||skin.id}/> </Button>  
        {#if skin.name}  
...
<p class="mb-2 ml=5 mt-5 text-secondary-text text-xl">Couleur:</p>
    <Input type="color" class="w-10 cursor-pointer" style="background-color:{col};" oninput={(ev)=>{ev.target.style.backgroundColor=ev.target.value; skinEditor.pickColor(layer,ev.target.value)}} value={col}/>
{:else}
<p class="mb-2 mt-5 text-secondary-text w-full text-xl">Couleurs:</p>
    {#each cat.colors as color}
    <Button class="ml-2" outline style=" background-color:{color}"  onclick={()=>skinEditor.pickColor(layer,color)} />
    {/each}
{/if}
{/if}
{@render children?.()}
