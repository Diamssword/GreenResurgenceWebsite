...
<TabItem title={cat.title}>
    ...
    <div class="my-3">
        ...
        {#if skin.id=="clear"}
            <Button class="ml-2 p-2 -translate-y-2" on:click={()=>cleanPart(m==1)}><img class="size-10" src="/skins/display/clear.svg" alt="Aucun"/></Button>    
            <Tooltip type="light">Aucun</Tooltip>
        {:else}
            <Button class="ml-2 p-0" on:click={()=>pickTexture(m==1,skin.id)}><img class="size-14" src="/skins/display/{skinEditor.getTexturePath(simpleLayer,skin.id)}" alt={skin.name||skin.id}/> </Button>  
            {#if skin.name}  
    ...
    <p class="mb-2 ml=5 mt-5 text-secondary-text text-xl">Couleur:</p>
        <Input type="color" class="w-10 cursor-pointer" style="background-color:{col};" on:input={(ev)=>{ev.target.style.backgroundColor=ev.target.value; pickColor(m==1,ev.target.value)}} value={col}/>
    {:else}
    <p class="mb-2 mt-5 text-secondary-text w-full text-xl">Couleurs:</p>
        {#each cat.colors as color}
        <Button class="ml-2" outline style=" background-color:{color}"  on:click={()=>pickColor(m==1,color)} />
        {/each}
    {/if}
    {/if}
    {#if splitted && m==0}
...
<div>
    ...
    <Toggle id="bras" checked={splitted} on:change={(e)=>switchMode(e.target.checked)} class="text-secondary-text cursor-pointer"></Toggle>
