import type { ModelType, RemoteImage, TextureSource } from "skinview-utils";
import {
	BoxGeometry,
	BufferAttribute,
	Color,
...
export class SkinObject extends Group {
	...
	constructor() {
		...
		this.modelListeners.push(() => {
			leftArmMesh.scale.x = 3.5+(this.slim?-0.25:0.5)
			leftArmMesh.scale.y = 12;
			leftArmMesh.scale.z = 4;
			setSkinUVs(leftArmBox, 32, 48, 3.5, 12, 4);
		});
	...
	set color(color:Color)
	{
		if(color)
		{
			this.layer1Material.color=color
			this.layer2Material.color=color
			this.layer1MaterialBiased.color=color;
			this.layer2MaterialBiased.color=color;
		}
		
	}
	get color():Color|undefined{
		return this.layer1Material.color;
	}
...
export class LayerInfo{
	...
	readonly skin?:RemoteImage | TextureSource
	color?:ColorRepresentation
	constructor(size:number,name:string,skin?:RemoteImage | TextureSource,external?:boolean,color?:number){
		this.size=size;
		this.name=name;
...
export class PlayerObject extends Group {
	readonly skins: {[name:string]:SkinObject};
	...
	constructor(layers:LayerInfo[]) {
		...
		layers.forEach(v=>{
			...
			l1.name=v.name;
			l1.position.y=8+v.size;
			l1.position.x=v.size
			l1.position.z=v.size
			if(v.color)
				l1.color=new Color(v.color)
			l1.scale.add(new Vector3(v.size,v.size,v.size))
			this.skins[v.name]=l1;
			this.add(l1)
		},this)
	...
	resetJoints(): void {
		for(let n in this.skins)
		{
			this.skins[n].resetJoints();
		}
		this.cape.rotation.x = CapeDefaultAngle;
		this.elytra.resetJoints();
	}
	forLayers(fn:(skin:SkinObject)=>void)
	{
		for(let n in this.skins)
			{
				fn(this.skins[n]);
