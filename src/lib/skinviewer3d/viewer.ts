...
import { Color, type ColorRepresentation, PointLight, EquirectangularReflectionMapping, Group, NearestFilter, PerspectiveCamera, Scene, Texture, Vector2, WebGLRenderer, AmbientLight, type Mapping, CanvasTexture, WebGLRenderTarget, FloatType, DepthTexture, Clock, Object3D, Vector3, Camera } from "three";
...
export class SkinViewer {
  ...
  public addLayer(layer: LayerInfo, slim?: boolean) {
    ...
    if (layer.color) l1.color = new Color(layer.color);
  ...
  setColor(layer: string, color: ColorRepresentation) {
    var l = this.playerObject.skins[layer];
    if (l) l.color = new Color(color);
  }
  loadSkin(layer: string, empty: null): void;
  loadSkin<S extends TextureSource | RemoteImage>(layer: string, source: S, options?: SkinLoadOptions): Promise<void>;

  async loadSkin(layer: string, source: TextureSource | RemoteImage | null, options: SkinLoadOptions = {}): Promise<void> {
    if (source === null) {
      this.resetSkin(layer);
    } else if (isTextureSource(source)) {
      let s = resizeTexture(source as any);
      if (options.side) s = splitFaceTexture(s, options.side == "right");
      ...
      var av = await loadImage(source);
      return await this.loadSkin(layer, av, options);
  ...
  private afterRenderResolvers: (() => void)[] = [];

  waitForRender() {
    return new Promise<void>((resolve) => {
      this.afterRenderResolvers.push(resolve);
    });
  }
  ...
  render(): void {
    ...
    for (const resolve of this.afterRenderResolvers) {
      resolve();
    }
    this.afterRenderResolvers.length = 0;
  }
  ...
  dispose(): void {
    ...
    this.playerObject.forLayers((v) => {
      this.resetSkin(v.name);
    });
    this.resetCape();
    this.resetEars();
